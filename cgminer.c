...
uint32_t pll3 = 510;
uint32_t STEP = 6;
uint32_t pllstart = 200;
uint32_t upperpll=650;
uint32_t lowerpll=500;

float gt_uppercent=0.05; //greater then pll0 & erro<5%, add pll
float gt_lowpercent=0.05; //greater then pll0 & error>5%, decrease pll
uint32_t gt_upstep=2;
uint32_t gt_downstep=5;
float lt_uppercent=0.01; //less then pll0 & erro<1%, add pll
float lt_lowpercent=0.3; //less then pll0 & error>30%, decrease pll
uint32_t lt_upstep=2;
uint32_t lt_downstep=5;

static int temp_fan_thr_id;
bool if_hashboard_cooldown = false;
bool enlog = false;
pthread_mutex_t fan_lock;

FILE* flog;
char curlogname[64];
bool b_h8cfg = false; //use as load config file
...
char g_miner_version[40] = {0};
char g_miner_compiletime[40] = {0};
char g_miner_type[40] = {0};
...
static const char def_h8conf[] = "h8.conf";
...
static char *set_int_0_to_1000(const char *arg, int *i)
{
	return set_int_range(arg, i, 0, 1000);
}

static char *set_int_0_to_6000(const char *arg, int *i)
{
	return set_int_range(arg, i, 0, 6000);
}
...
static char *set_int_0_to_400(const char *arg, int *i)
{
	return set_int_range(arg, i, 0, 400);
}
...
static struct opt_table opt_config_stu_table[] = {
#if defined(USE_H8) || defined(USE_U6)
		OPT_WITH_ARG("--pll0",
				 set_int_0_to_1000, opt_show_intval, &pll0,
				 "set pll(Mhz) for pll0"),
		OPT_WITH_ARG("--pll1",
				 set_int_0_to_1000, opt_show_intval, &pll1,
				 "set pll(Mhz) for pll1"),
		OPT_WITH_ARG("--pll2",
				 set_int_0_to_1000, opt_show_intval, &pll2,
				 "set pll(Mhz) for pll2"),
	
		OPT_WITH_ARG("--pll3",
				 set_int_0_to_1000, opt_show_intval, &pll3,
				 "set pll(Mhz) for pll3"),
	
		OPT_WITH_ARG("--upperpll",
				 set_int_0_to_1000, opt_show_intval, &upperpll,
				 "set pll(Mhz) for upperpll"),
				 
		OPT_WITH_ARG("--lowerpll",
				 set_int_0_to_1000, opt_show_intval, &lowerpll,
				 "set pll(Mhz) for lowerpll"),
				 
		OPT_WITH_ARG("--gt_upstep",
				 set_int_0_to_1000, opt_show_intval, &gt_upstep,
				 "set pll(Mhz) for upstep"),
		OPT_WITH_ARG("--gt_downstep",
				 set_int_0_to_1000, opt_show_intval, &gt_downstep,
				 "set pll(Mhz) for downstep"),
		OPT_WITH_ARG("--lt_upstep",
				 set_int_0_to_1000, opt_show_intval, &lt_upstep,
				 "set pll(Mhz) for upstep"),
		OPT_WITH_ARG("--lt_downstep",
				 set_int_0_to_1000, opt_show_intval, &lt_downstep,
				 "set pll(Mhz) for downstep"),
		OPT_WITH_ARG("--gt_uppercent",
				 opt_set_floatval, &opt_show_floatval, &gt_uppercent,
				 "Set percentage of adding pll 0~1"),		 
		OPT_WITH_ARG("--gt_lowpercent",
				 opt_set_floatval, &opt_show_floatval, &gt_lowpercent,
				 "Set percentage of reducing pll 0~1"),   
	
		OPT_WITH_ARG("--lt_uppercent",
				 opt_set_floatval, &opt_show_floatval, &lt_uppercent,
				 "Set percentage of adding pll 0~1"),		 
		OPT_WITH_ARG("--lt_lowpercent",
				 opt_set_floatval, &opt_show_floatval, &lt_lowpercent,
				 "Set percentage of reducing pll 0~1"),
				 
		OPT_WITH_ARG("--fan0",
				 set_int_0_to_6000, opt_show_intval, &fan0,
				 "set fans speed"), 
		OPT_WITH_ARG("--fan1",
				 set_int_0_to_6000, opt_show_intval, &fan1,
				 "set fans speed"),
				 
		OPT_WITH_ARG("--workvolt",
				 opt_set_floatval, &opt_show_floatval, &workvolt,
				 "set work voltage for hashboard 12.0v ~14.0v"),	
	
		OPT_WITH_ARG("--startvol",
				 opt_set_floatval, &opt_show_floatval, &startvol,
				 "set  start voltage for hashboard 12.0v ~14.0v"),	
		OPT_WITH_ARG("--pllstep",
				 set_int_0_to_200, opt_show_intval, &STEP,
				 "set pll add value each step"),	
		OPT_WITH_ARG("--pllstart",
				 set_int_0_to_400, opt_show_intval, &pllstart,
				 "set pll init value"), 	
		OPT_WITHOUT_ARG("--enlog",
				opt_set_bool, &enlog,
				"Enable log, default: disabled"),		 
#endif
	OPT_ENDTABLE
...
static char *parse_config(json_t *config, bool fileconf)
{
	...
	if(!b_h8cfg) opt = opt_config_table;
	else         opt = opt_config_stu_table;
...
static void calc_midstate(struct pool *pool, struct work *work)
{
	...
	if (pool->vmask) {
		...
		applog(LOG_INFO,"midstate1=0x%x",work->midstate1);

		memcpy(work->data, &(pool->vmask_001[4]), 4);
		//work->version1 = pool->vmask_001[4];
		flip64(data32, work->data);
		sha256_init(&ctx);
		sha256_update(&ctx, data, 64);
		cg_memcpy(work->midstate2, ctx.h, 32);
		endian_flip32(work->midstate2, work->midstate2);
		applog(LOG_INFO,"midstate2=0x%x",work->midstate2);
		...
		applog(LOG_INFO,"midstate3=0x%x",work->midstate3);


		memcpy(work->data, &(pool->vmask_001[0]), 4);
		applog(LOG_INFO,"case4=0x%x",work->data);
	}
#endif	
	...
	//applog(LOG_ERR, "pversion = %08x", *pversion);
		...
		work->version1 = (work->version|pool->vmask_val);

		applog(LOG_INFO,"here version=0x%x,version1=0x%x,vmask_val=0x%x",work->version,work->version1,pool->vmask_val);
...
#if defined(USE_H8) || defined(USE_U6)
static void *temp_fan_thread(void *userdata)
{
	uint8_t i;
	uint8_t re[8];
	int ret;
	...
	int speed1, speed2;
	float temp;
	#define START_UP_TEMP	35.0

	applog(LOG_ERR, "Start temp_fan_thread ...");

	pthread_detach(pthread_self());
	pthread_setcanceltype(PTHREAD_CANCEL_ASYNCHRONOUS, NULL);

	RenameThread("TempFan");
	...
	freq_vendor = Get_fan_fre(11);
	speed1 = freq_vendor * 60 / 2;
	cgsleep_ms(1000);
	freq_vendor = Get_fan_fre(12);
	speed2 = freq_vendor * 60 / 2;

	
	if ((speed1 + speed2) < 4000)
	{
		printf("xxx below standard----------------------------- \n");
	}
...
//	power_on_off(1);
	...
	if_hashboard_cooldown = true; //开始配置挖矿

	while(1)
	{
		mutex_lock(&fan_lock);

	//	applog(LOG_ERR,"mylog fan thread \n");
		stu_board_fan_update();
		cgsleep_ms(5000);
		mutex_unlock(&fan_lock);
	}

	applog(LOG_ERR, "End temp_fan_thread ...");
}
		...
		mutex_unlock(&sshare_lock);
			...
			if (pool->vmask) {
				snprintf(s, sizeof(s),
					...
					pool->rpc_user, work->job_id, nonce2hex, work->ntime, noncehex, work->vmask_str, sshare->id);
				applog(LOG_INFO,"here ready submit %s",s);
			} else {
				snprintf(s, sizeof(s),
...
{
#ifdef USE_U6
	uint32_t *phash =(uint32_t *)work->hash;
	uint32_t *pdata = (uint32_t *)work->data;
	uint32_t *ptarget = (uint32_t *)work->target;
	uint32_t endiance[20];
	
	for (int k=0;k<20; k++)
			endiance[k] = swab32(pdata[k]);
	
	x11hash(work->hash, endiance);

	
#if 0
	applog(LOG_ERR, "hashval:");
	for (int i=0; i<8; i++)
	{
		printf("%08x ", phash[i]);
	}
	printf("\n");
/*	applog(LOG_ERR, "endiance:");
	for (int i=0; i<20; i++)
	{
		printf("%08x ", endiance[i]);
	}
	printf("\n");
	applog(LOG_ERR, "target:");
	for (int i=0; i<8; i++)
	{
...
*/	
...
#if 1 //debug
	if ((phash[7]&0xffff0000) == 0x00)
	{
		//applog(LOG_ERR, "#####right nonce###### !");
		return true;
	}
#endif
	...
	{
		//applog(LOG_ERR, "Share above target, diff = %f", work->sdiff);
		return false;
	}
...
bool submit_tested_work(struct thr_info *thr, struct work *work)
{
	...
	if (!fulltest(work->hash, work->target)) {
		applog(LOG_ERR, "%s %d: Share above target 0x%064x -- 0x%064x", thr->cgpu->drv->name,
		       thr->cgpu->device_id,work->hash,work->target);
		return false;
	}
...
bool submit_nonce(struct thr_info *thr, struct work *work, uint32_t nonce)
{
	if (new_nonce(thr, nonce) && test_nonce(work, nonce))
		submit_tested_work(thr, work); //debug
	else {
		inc_hw_errors(thr);
		return false;
	}
...
static bool CheckForAnotherInstance()
{
    int fd;
    struct flock fl;
    fd = open ( "/tmp/cgminer.lock", O_RDWR | O_CREAT, 0664);
...
//        cout << "Failed to find first file!\n";
    ...
    do
    {
		...
		{
			//cout << findData.name << "\t" << findData.size<<"  "<<findData.time_create<<endl;
			int a = time(NULL);
			int b = findData.time_create;
			if(time(NULL)-findData.time_create>DELPOINT)
			{
				//cout<<"delete old log file"<<" "<<remove(findData.name)<<endl;	
...
#define DELPOINT 60*60*24*7  //秒
    ...
    if (dirp != NULL) 
	{
       while ((direntp = readdir(dirp)) != NULL)
       {
			...
			char* suff = p +(strlen(p)-4);
			if(strcmp(suff,".log") != 0) continue;
			char file[64]={0};
			sprintf(file,"%s%s",dir,direntp->d_name);
			printf("filename===%s\n",file);
			result = stat(file,&buf);
			if(result !=0) printf("stat err==%s",strerror(errno));  
			else
			{
				if(time(NULL)-buf.st_ctime>DELPOINT)
				{
					if(0 == remove(file))
						printf("remove %s finish\n",direntp->d_name);
					else
...
int main(int argc, char *argv[])
{
	...
	char *s;
		...
		if (!CheckForAnotherInstance())
    		{    
       	 		applog(LOG_ERR,"Another cgminer is running");
				fclose(flog);
        		abort();
        		return 0;
    		}    
		...
		#if 0
		applog(LOG_ERR, "fans full speed 20s");
		stu_board_fan_pwm_set(100.0, 100.0);
		sleep(30);
		#endif
		...
		power_on_off(1);
		sleep(1);
		pthread_t logthreadid = 0;
		pthread_create(&logthreadid, NULL, logcheckthread, NULL);
	#endif
...
#if defined(USE_H8) || defined(USE_U6)
		if ((startvol<12.0)||(startvol>14.0))
		{
			startvol=13.3;
		}
		if ((workvolt<12.0)||(workvolt>14.0))
		{
			workvolt=13.3;
		}
	
		changevol = startvol;
		stu_board_set_voltage(startvol);
		sleep(1);
#endif

	printf("startvol = %0.2fv, workvolt=%0.2fv\n", startvol, workvolt);
	printf("upperpll=%d, lowerpll=%d\n", upperpll, lowerpll);
	printf("gt_upstep=%d, gt_downstep=%d\n", gt_upstep, gt_downstep);
	printf("lt_upstep=%d, lt_downstep=%d\n", lt_upstep, lt_downstep);
	printf("gt_uppercent=%0.2f, gt_lowpercent=%0.2f\n", gt_uppercent, gt_lowpercent);
	printf("lt_uppercent=%0.2f, lt_lowpercent=%0.2f\n", lt_uppercent, lt_lowpercent);

	
	system("echo > /home/root/reset.log");
	system("echo > /home/root/volt.log");

	//load h8.conf begin
	b_h8cfg = true;
	load_default_config();
	b_h8cfg = false; //load stu.conf after set defalut status
	printf("startvol = %0.2fv, workvolt=%0.2fv\n", startvol, workvolt);
	printf("upperpll=%d, lowerpll=%d\n", upperpll, lowerpll);
	printf("gt_upstep=%d, gt_downstep=%d\n", gt_upstep, gt_downstep);
	printf("lt_upstep=%d, lt_downstep=%d\n", lt_upstep, lt_downstep);
...
//close fan rember open it wangtao begin	
#if 0
	mutex_init(&fan_lock);
	temp_fan_thr_id = 6;
	thr = &control_thr[temp_fan_thr_id];
	if(thr_info_create(thr, NULL, temp_fan_thread, thr))
		early_quit(1, "temp_fan thread create failed");
#endif
//end
#ifdef USE_USBUTILS
	hotplug_thr_id = 6;
