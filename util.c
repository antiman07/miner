...
#ifdef USE_H8
static inline void get_vmask(struct pool *pool, char *bbversion)
{
}
...
static bool set_vmask(struct pool *pool, json_t *val)
{
	...
	snprintf(pool->vmask_str, sizeof(pool->vmask_str), "%s", version_mask);

	mask = strtol(version_mask, NULL, 16);
	if (!mask)
		return false;
	//snprintf(pool->vmask_str, sizeof(pool->vmask_str), "%f", version_mask);
	pool->vmask_val= mask;
	applog(LOG_ERR, "*** vmask_str=%s, vmask_val=%08x", pool->vmask_str, pool->vmask_val);
...
static void get_vmask(struct pool *pool, char *bbversion)
{
	...
	//debug info
	applog(LOG_INFO,"bbversion=%s,bversion=%d",bbversion,bversion);
	for(i=0;i<16;i++)
	applog(LOG_INFO,"vmask_001[%d]=0x%08x",i,pool->vmask_001[i]);
	for(i=0;i<16;i++)
	applog(LOG_INFO,"vmask_002[%d]=%s",i,pool->vmask_002[i]);
	for(i=0;i<4;i++)
	applog(LOG_INFO,"vmask_003[%d]=0x%08x",i,pool->vmask_003[i]);
}
...
static bool configure_stratum_mining(struct pool *pool)
{
	...
	applog(LOG_INFO, "send req version rolling=%s", s);
