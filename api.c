...
#include "stu-device.h"
  ...
  defined(USE_HASHFAST) || defined(USE_BITFURY) || defined(USE_BITFURY16) || defined(USE_BLOCKERUPTER) || defined(USE_KLONDIKE) || \
	...
	defined(USE_AVALON7) || defined(USE_AVALON8) || defined(USE_BITMAIN_SOC)||defined(USE_H8) || defined(USE_U6)
...
#define MSG_MISPLL 128
#define MSG_MISVOL 129
#define MSG_MISFAN 130
...
static void ascstatus(struct io_data *io_data, int asc, bool isjson, bool precom)
{
	...
	if (numasc > 0 && asc >= 0 && asc < numasc) {
		...
		struct stu_chain *chain =cgpu->device_data;
		//float temp = cgpu->temp;
		float volt = g_borad_volt[chain->chain_id];
		double freq = g_board_freq[chain->chain_id];
		float temp = g_board_temp[chain->chain_id];
		double dev_runtime;
		...
		root = api_add_int(root, "ASC", &chain->chain_id, false);
		root = api_add_string(root, "Name", cgpu->drv->name, false);
		root = api_add_int(root, "ID", &(cgpu->device_id), false);
		root = api_add_string(root, "Enabled", enabled, false);
		root = api_add_string(root, "Status", status, false);
		root = api_add_temp(root, "Temperature", &temp, false);
		root = api_add_volts(root, "volt", &volt, false);
		root = api_add_int(root, "ChipNum", &chip_num, false);
		root = api_add_int(root, "Frequency", &chain->newpll, false);
		...
		root = api_add_int(root, "Reboot Count", &chain->reboot_cnt, false);
...
static void poolstatus(struct io_data *io_data, __maybe_unused SOCKETTYPE c, __maybe_unused char *param, bool isjson, __maybe_unused char group)
{
	...
	for (i = 0; i < total_pools; i++) {
		...
		tblock = localtime(&pool->last_share_time);
		...
		root = api_add_string(root, "Last Share Time", asctime(tblock), false);
...
static void summary(struct io_data *io_data, __maybe_unused SOCKETTYPE c, __maybe_unused char *param, bool isjson, __maybe_unused char group)
{
	...
	#ifdef USE_U6
		api_add_string(root, "Miner type", "dash", false);
	#endif

	#ifdef USE_H8
		api_add_string(root, "Miner type", "btc", false);
	#endif

	#if defined(USE_H8) || defined(USE_U6)
		uint32_t speed1=0;
		uint32_t speed2=0;
		uint32_t fanNum = 2;
		speed1 = Get_fan_speed(1);
		speed2 = Get_fan_speed(2);
		
		root = api_add_uint(root, "Fan Num", &fanNum, false);
		root = api_add_uint(root, "Fan1", &speed1, false);
		root = api_add_uint(root, "Fan2", &speed2, false);
	#endif
...
static void minerstats(struct io_data *io_data, __maybe_unused SOCKETTYPE c, __maybe_unused char *param, bool isjson, __maybe_unused char group)
{
	...
	root = api_add_string(root, "StuMiner", VERSION, false);
...
static void setfan(struct io_data *io_data, __maybe_unused SOCKETTYPE c, char *param, bool isjson, __maybe_unused char group)
{
	uint32_t fans_val[2];
	int i = 0;
	char *ptr=NULL;
	...
	if (param == NULL || *param == '\0') {
		message(io_data, MSG_MISFAN, 0, NULL, isjson);
		return;
	}
	...
	printf("set setfan %d, %d\n", fan0,fan1);
	stu_board_fan_pwm_set(fan0, fan1);

	if (isjson)
		io_put(io_data, JSON_ACTION "\"" "success" "\"");
	else
		io_put(io_data, "success");
}
static void setvolt(struct io_data *io_data, __maybe_unused SOCKETTYPE c, char *param, bool isjson, __maybe_unused char group)
{
	uint32_t volt_val[3];
	int i = 0;
	char *ptr=NULL;
	...
	if (param == NULL || *param == '\0') {
		message(io_data, MSG_MISVOL, 0, NULL, isjson);
		return;
	}
	...
	g_borad_volt[0]= volt_val[0];
	//g_borad_volt[1]= volt_val[1];
	//g_borad_volt[2]= volt_val[2];

	printf("set voltage %d, %d, %d\n", volt0, volt1, volt2);
	stu_board_set_voltage(volt0);
	#endif
...
static void setpll(struct io_data *io_data, __maybe_unused SOCKETTYPE c, char *param, bool isjson, __maybe_unused char group)
{
	uint32_t pll_val[3];
	int i = 0;
	char *ptr=NULL;
	...
	if (param == NULL || *param == '\0') {
		message(io_data, MSG_MISPLL, 0, NULL, isjson);
		return;
	}
	...
	g_board_freq[0] = pll_val[0];
	g_board_freq[1] = pll_val[1];
	g_board_freq[2] = pll_val[2];

	printf("set pll %d, %d, %d\n", pll0, pll1, pll2);
...
*/
static void writereg(struct io_data *io_data, __maybe_unused SOCKETTYPE c, char *param, bool isjson, __maybe_unused char group)
{
	json_error_t json_err;
	json_t *json_config;
	json_t *json_val,*json_val_1;
	char* pchainid,*pchipid,*pregid,*pbuff;
	uint8_t *asic_cmd = NULL;
	uint8_t chain_id;
	struct cgpu_info *cgpu;
	...
	char jsonbuf[1024]="0";
	char strhexmsg[1024]="0";

	/*if (param == NULL || *param == '\0') {
		goto WrongParam;
	}*/

	json_val =(json_t*)param;

	if(json_is_object(json_val))
	{
		json_val_1 = json_object_get(json_val, "chainid");
		pchainid = (char *)json_string_value(json_val_1);
	
		json_val_1 = json_object_get(json_val, "chipid");
		pchipid = (char *)json_string_value(json_val_1 );
		
		json_val_1 = json_object_get(json_val, "regid");
		pregid = (char *)json_string_value(json_val_1);
		
		json_val_1 = json_object_get(json_val, "buff"); 								
		pbuff = (char *)json_string_value(json_val_1);
	}
	else  goto WrongParam;
	
	sprintf(strhexmsg,"%s86%s00%s%s",pchainid,pchipid,pregid,pbuff);
	if ((0==strlen(strhexmsg)) || (0!=(strlen(strhexmsg)%2)) ||(0 == (strlen(strhexmsg)/2)%2))
		param = NULL;

	applog(LOG_DEBUG, "get cmd:%s", strhexmsg);
	
	asic_cmd = calloc(1+strlen(strhexmsg)/2, 1);
	hex2bin(asic_cmd, strhexmsg, strlen(strhexmsg)/2);
	chain_id = asic_cmd[0];
	...
	swap_data(asic_cmd+1, (strlen(strhexmsg)/2)-1);

	applog(LOG_ERR , "send :");
	for (int i=1; i<((strlen(strhexmsg)/2)); i++)
	{
		printf("%02x ", asic_cmd[i]);
	}
	printf("\n");

	mutex_lock(&chain->lock);
	spi_send_data(chain->spi_ctx, asic_cmd+1, (strlen(strhexmsg)/2)-1);	
	flush_spi(chain->spi_ctx);
...
WrongParam:
	if (isjson)
		io_put(io_data, JSON_ACTION "\"" "wrong param" "\"");
	else
		io_put(io_data, "wrong param");
	return;
}

static void setcmd(struct io_data *io_data, __maybe_unused SOCKETTYPE c, char *param, bool isjson, __maybe_unused char group)
{
	uint32_t pll_val[3];
	int i = 0;
	char *ptr=NULL;
	...
	if ((0==strlen(param)) || (0!=(strlen(param)%2)) ||(0 == (strlen(param)/2)%2))
		param = NULL;
	...
	applog(LOG_ERR, "get cmd:%s", str);
	asic_cmd = calloc(1+strlen(param)/2, 1);
	hex2bin(asic_cmd, param, strlen(param)/2);
	...
	swap_data(asic_cmd+1, (strlen(param)/2)-1);

	/*applog(LOG_ERR , "send :");
	for (int i=1; i<((strlen(param)/2)); i++)
	{
		printf("%02x ", asic_cmd[i]);
	}
	printf("\n");*/
	...
	mutex_lock(&chain->lock);
	spi_send_data(chain->spi_ctx, asic_cmd+1, (strlen(param)/2)-1);	
	flush_spi(chain->spi_ctx);
	mutex_unlock(&chain->lock);
	free(asic_cmd);
...
//ÃüÁî{"command":"readreg","parameter":"00120a"}
//±íÊ¾read spi0, chip_18, reg_10,
static void readreg(struct io_data *io_data, __maybe_unused SOCKETTYPE c, char *param, bool isjson, __maybe_unused char group)
{
	uint32_t pll_val[3];
	int i = 0;
	char *ptr=NULL;
	...
	if ((6!=strlen(param)) || (0!=(strlen(param)%2)))
		param = NULL;
	...
	if((chain_id>2) ||(regAddr>13)||(chipid>ASIC_CHIP_NUM))
		goto WrongParam;
	...
	regLen = get_regLenEx(regAddr);
	mutex_lock(&chain->lock);
	ret = cmd_read_register(chain->spi_ctx,  chipid, regAddr, regLen, regBuf);
	mutex_unlock(&chain->lock);
	...
	for (i=0; i<regLen; i++)
	{
		sprintf(strbuf+offset, "%02x", regBuf[i]);
		offset +=2;
	}
	...
	//root = api_add_string(root, "CGMiner", VERSION, false);
	//root = api_add_const(root, "API", APIVERSION, false);
	root = api_add_string(root, "data", strbuf, false);
...
ReadError:	
	if (isjson)
		io_put(io_data, JSON_ACTION "\"" "read error" "\"");
	else
		io_put(io_data, "read error");
...
static void coreacc(struct io_data *io_data, __maybe_unused SOCKETTYPE c, __maybe_unused char *param, bool isjson, __maybe_unused char group)
{
	bool io_open = false;
	int devcount = 0;
	int numasc = 0;
	...
	char buff[24]={0};
#if 0
	fd = open("/home/root/corestat.txt", O_RDWR|O_TRUNC|O_CREAT, 0664);
	if (fd < 0)
	{
		if (isjson)
		io_put(io_data, JSON_ACTION "\"" "fail" "\"");
	else
		io_put(io_data, "fail");
	}
	else
	{
		for (i = 0; i<total_devices; i++)
		{
			...
			sprintf(buff, "hashboard_spi%d:\n\n", chain->chain_id);
			write(fd, buff, strlen(buff));

			for (j=0; j<chain->num_active_chips; j++)
			{
				memset(buff, 0, sizeof(buff));
				sprintf(buff, "chip_%d:\n", j);
				write(fd, buff, strlen(buff));
				
				chip = &chain->chips[j];

				for (k=0; k<TOTAL_CORE; k++)
				{
					//chip->core_acc[k]
					memset(buff, 0, sizeof(buff));
					sprintf(buff, "%d ", chip->core_acc[k]);
					write(fd, buff, strlen(buff));
				}
				
				memset(buff, 0, sizeof(buff));
				sprintf(buff, "\n wrong core id %d \n\n", chip->core_acc[TOTAL_CORE]);
				write(fd, buff, strlen(buff));
				usleep(1);
			}
		}
...
static void getchippll(struct io_data *io_data, int asc, bool isjson, bool precom)
{
	struct api_data *root = NULL;
	char *enabled;
	char *status;
	...
	if (numasc > 0 && asc >= 0 && asc < numasc) {
		...
		root = api_add_int(root, "ASC", &chain->chain_id, false);
		root = api_add_string(root, "Name", basename(chain->devname), false);
		root = api_add_int(root, "ID", &(chain->chain_id), false);
		root = api_add_int(root, "Total chips", &(total_chip), false);
		
		for (i=0; i<ASIC_CHIP_NUM; i++)
		{
			snprintf(buf, sizeof(buf), "%d_pll", i+1);
			root = api_add_uint(root, buf, &(chain->chips[i].chipNewpll), false);
		}
...
static void chippll(struct io_data *io_data, __maybe_unused SOCKETTYPE c, __maybe_unused char *param, bool isjson, __maybe_unused char group)
{
	bool io_open = false;
	int devcount = 0;
	int numasc = 0;
	...
	if (numasc > 0) {
		for (i = 0; i < numasc; i++) {
			getchippll(io_data, i, isjson, isjson && devcount > 0);
			devcount++;
		}
	}
#endif
...
/*calculate each pll's (0~3) distribution on core of chips*/
/*static void chippll(struct io_data *io_data, __maybe_unused SOCKETTYPE c, __maybe_unused char *param, bool isjson, __maybe_unused char group)
{
	bool io_open = false;
	int devcount = 0;
	...
	int fd;
	char buff[24]={0};
	uint32_t avPll;//average
	uint32_t BoardavPll=0;

	fd = open("/home/root/chippll.txt", O_RDWR|O_TRUNC|O_CREAT, 0664);
	if (fd < 0)
	{
		if (isjson)
		io_put(io_data, JSON_ACTION "\"" "fail" "\"");
	...
	else
	{
		for (i = 0; i<total_devices; i++)
		{
			...
			{
				memset(buff, 0, sizeof(buff));
				sprintf(buff, "chip_%d:", j+1);
				write(fd, buff, strlen(buff));
				
				chip = &chain->chips[j];
				
				memset(buff, 0, sizeof(buff));
				avPll = chip->pllCnt[0]*chip->pll[0]+ chip->pllCnt[1]*chip->pll[1] 
					+ chip->pllCnt[2]*chip->pll[2] +chip->pllCnt[3]*chip->pll[3];
				
				BoardavPll += avPll;
				avPll /= TOTAL_CORE;
				sprintf(buff, "%d %d %d %d (%d)\n", chip->pllCnt[0],chip->pllCnt[1],chip->pllCnt[2],chip->pllCnt[3], avPll);
				write(fd, buff, strlen(buff));
				usleep(1);
			}
			memset(buff, 0, sizeof(buff));
			sprintf(buff, "hashboard_spi average pll: %d\n",chain->avpll);
			write(fd, buff, strlen(buff));
		}
...
static void chipaccept(struct io_data *io_data, int asc, bool isjson, bool precom)
{
	struct api_data *root = NULL;
	char *enabled;
	char *status;
	...
	if (numasc > 0 && asc >= 0 && asc < numasc) {
		...
		for (i=0; i<ASIC_CHIP_NUM; i++)
		{
			snprintf(buf, sizeof(buf), "%d_accept", i+1);
			root = api_add_uint(root, buf, &(chain->chips[i].chip_nonces_found), false);
		}
...
static void chipacc(struct io_data *io_data, __maybe_unused SOCKETTYPE c, __maybe_unused char *param, bool isjson, __maybe_unused char group)
{
	bool io_open = false;
	int devcount = 0;
	int numasc = 0;
	...
	if (numasc > 0) {
		for (i = 0; i < numasc; i++) {
			chipaccept(io_data, i, isjson, isjson && devcount > 0);
			devcount++;
		}
	}
#endif
...
static void chip_hardware_error(struct io_data *io_data, int asc, bool isjson, bool precom)
{
	struct api_data *root = NULL;
	char *enabled;
	char *status;
	...
	if (numasc > 0 && asc >= 0 && asc < numasc) {
		...
		for (i=0; i<ASIC_CHIP_NUM; i++)
		{
			snprintf(buf, sizeof(buf), "%d_hwe", i+1);
			root = api_add_uint(root, buf, &(chain->chips[i].hw_errors), false);
		}
...
static void chiphwe(struct io_data *io_data, __maybe_unused SOCKETTYPE c, __maybe_unused char *param, bool isjson, __maybe_unused char group)
{
	bool io_open = false;
	int devcount = 0;
	int numasc = 0;
	...
	if (numasc > 0) {
		for (i = 0; i < numasc; i++) {
			chip_hardware_error(io_data, i, isjson, isjson && devcount > 0);
			devcount++;
		}
	}
#endif
...
static void chip_crcError(struct io_data *io_data, int asc, bool isjson, bool precom)
{
	struct api_data *root = NULL;
	char *enabled;
	char *status;
	...
	if (numasc > 0 && asc >= 0 && asc < numasc) {
		...
		for (i=0; i<ASIC_CHIP_NUM; i++)
		{
			snprintf(buf, sizeof(buf), "%d_crcErr", i+1);
			root = api_add_uint(root, buf, &(chain->chips[i].crc_error), false);
		}
...
static void crcerror(struct io_data *io_data, __maybe_unused SOCKETTYPE c, __maybe_unused char *param, bool isjson, __maybe_unused char group)
{
	bool io_open = false;
	int devcount = 0;
	int numasc = 0;
	...
	if (numasc > 0) {
		for (i = 0; i < numasc; i++) {
			chip_crcError(io_data, i, isjson, isjson && devcount > 0);
			devcount++;
		}
	}
#endif
...
static void chip_crcRight(struct io_data *io_data, int asc, bool isjson, bool precom)
{
	struct api_data *root = NULL;
	char *enabled;
	char *status;
	...
	if (numasc > 0 && asc >= 0 && asc < numasc) {
		...
		for (i=0; i<ASIC_CHIP_NUM; i++)
		{
			snprintf(buf, sizeof(buf), "%d_crcRight", i+1);
			root = api_add_uint(root, buf, &(chain->chips[i].crc_right), false);
		}
...
static void crcright(struct io_data *io_data, __maybe_unused SOCKETTYPE c, __maybe_unused char *param, bool isjson, __maybe_unused char group)
{
	bool io_open = false;
	int devcount = 0;
	int numasc = 0;
	...
	if (numasc > 0) {
		for (i = 0; i < numasc; i++) {
			chip_crcRight(io_data, i, isjson, isjson && devcount > 0);
			devcount++;
		}
	}
#endif
...
static void chiptest_result(struct io_data *io_data, int asc, bool isjson, bool precom)
{
	struct api_data *root = NULL;
	char *enabled;
	char *status;
	...
	if (numasc > 0 && asc >= 0 && asc < numasc) {
		...
		for (i=0; i<ASIC_CHIP_NUM; i++)
		{
			snprintf(buf, sizeof(buf), "%d_test", i+1);
			root = api_add_uint8(root, buf, &(chain->chips[i].if_chip_ok), false);
		}
...
static void chiptest(struct io_data *io_data, __maybe_unused SOCKETTYPE c, __maybe_unused char *param, bool isjson, __maybe_unused char group)
{
	bool io_open = false;
	int devcount = 0;
	int numasc = 0;
	...
	if (numasc > 0) {
		for (i = 0; i < numasc; i++) {
			chiptest_result(io_data, i, isjson, isjson && devcount > 0);
			devcount++;
		}
	}
#endif
...
static void getchipft(struct io_data *io_data, int asc, bool isjson, bool precom)
{
	struct api_data *root = NULL;
	char *enabled;
	char *status;
	...
	if (numasc > 0 && asc >= 0 && asc < numasc) {
		...
		for (i=0; i<ASIC_CHIP_NUM; i++)
		{
			snprintf(buf, sizeof(buf), "%d_ft", i+1);
			root = api_add_uint8(root, buf, &(chain->chips[i].ft_pass_cnt), false);
		}
...
static void chipft(struct io_data *io_data, __maybe_unused SOCKETTYPE c, __maybe_unused char *param, bool isjson, __maybe_unused char group)
{
	bool io_open = false;
	int devcount = 0;
	int numasc = 0;
	...
	if (numasc > 0) {
		for (i = 0; i < numasc; i++) {
			getchipft(io_data, i, isjson, isjson && devcount > 0);
			devcount++;
		}
	}
#endif
...
static void dumpreg(struct io_data *io_data, __maybe_unused SOCKETTYPE c, __maybe_unused char *param, bool isjson, __maybe_unused char group)
{
	bool io_open = false;
	int devcount = 0;
	int numasc = 0;
	...
	fd = open("/home/root/chipreg.txt", O_RDWR|O_TRUNC|O_CREAT, 0664);
	if (fd < 0)
	{
		if (isjson)
		io_put(io_data, JSON_ACTION "\"" "fail" "\"");
	...
	else
	{
		...
		for (i = 0; i<total_devices; i++)
		{
			...
			for (j=0; j<chain->num_active_chips; j++)
			{
				...
				{
					#ifdef USE_U6
						if (8==n) continue; // u6 has not reg8
					#endif
					regLen = get_regLenEx(n);
					memset(buff, 0, sizeof(buff));
					memset(reg_buf,0,sizeof(reg_buf));
					mutex_lock(&chain->lock);
					ret = cmd_read_register(chain->spi_ctx,  j+1, n , regLen, reg_buf);
					mutex_unlock(&chain->lock);
					if (ret)
					{	
						offset=0;
						memset(tmpbuf, 0, sizeof(tmpbuf));
						for (int m=0; m<regLen; m++)
						{
							sprintf(tmpbuf+offset, "%02x", reg_buf[m]);
							offset+=2;
						}
						sprintf(buff, "    reg%d:%s\n", n, tmpbuf);
					}
					else
					{
						sprintf(buff, "    reg%d:%s\n", n, "error");
...
struct CMDS {
	...
	{ "lockstats",		lockstats,	false,	true },
	{"setfan",  setfan,	false, false},
	{"setvolt", setvolt,	false, false},
	{"setpll", setpll,	false, false},
	{"chipacc", chipacc,false, false},
	{"chiphwe", chiphwe,	false, false},
	{"coreacc", coreacc,	false, false},
	{"crcerror", crcerror,	false, false},
	{"crcright", crcright,	false, false},
	{"chippll", chippll,	false, false},
	{"chipft", chipft, false, false},
	{"chiptest", chiptest,	false, false},
	{"dumpreg", dumpreg, false, false},
	{"setcmd", setcmd, false, false},
	//{"readreg", readreg, true, false},
	{"readreg", readreg, false,true},
	{"writereg", writereg, false, true},
					...
					if (!json_is_object(json_config)) {
						...
						if (json_val == NULL) {
							...
							if (!json_is_string(json_val)) {
								...
								if(strcmp(cmd,"readreg") == 0 || strcmp(cmd,"writereg") == 0) 
								{
									printf("cur cmd=======%s\n",cmd);
								...
								if(json_is_object(json_val))
									param = (char*)json_val;
								else if (json_is_string(json_val))
